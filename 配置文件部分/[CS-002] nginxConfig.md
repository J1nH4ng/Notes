...
```nginx
# 定义详细的日志格式
log_format detailed '$remote_addr - $remote_user [$time_local] '  # 记录客户端IP和访问时间                                     '"$request" $status $body_bytes_sent '        # 记录请求信息、状态码和发送字节数                    '"$http_referer" "$http_user_agent" '        # 记录来源页面和用户代理                    '$request_time $upstream_response_time';      # 记录请求处理时间和上游响应时间
...
```nginx
# 设置错误日志级别为warn# 可选级别: debug, info, notice, warn, error, crit, alert, emerg
